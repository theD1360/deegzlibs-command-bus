...
Instead of defining a message class and a handler class, you can use **`@router.command()`**. The decorator:

1. Builds a **CommandMessage** subclass from the function's parameters (names and type annotations).
...
So you can send commands like this:
...
- **Generated message class:** Attached as **`func._command_message_class`**. The class is also registered on the function's module so repr-based parsers can resolve it when dispatching from the queue.
